...
class AppStore {
  ...
  addToGallery = async () => {
    ...
    this.galleryList = [...this.galleryList, uuid];
...
reaction(
  (): IGallery => {
    const { galleryItems, galleryList } = store;
  ...
  (gallery) => {
    const { items, list } = gallery;

    const itemsToStore: Record<string, IGalleryItem> = list.reduce(
      (acc, key) => {
        const { id, params, text } = items[key];
